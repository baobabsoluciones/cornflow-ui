...
vi.mock('@/components/input-data/InputOutputDataTable.vue', () => ({
  default: {
    name: 'InputOutputDataTable',
    template: '<div data-testid="input-output-data-table">InputOutputDataTable</div>',
    props: ['type', 'execution']
  }
}))
...
const createWrapper = (mockSelectedExecution = null) => {
  ...
  const wrapper = mount(OutputDataView, {
    global: {
...
describe('OutputDataView', () => {
  ...
  describe('Component Rendering', () => {
    ...
    test('renders InputOutputDataTable when execution has solution', () => {
      const mockExecution = {
        id: 1,
        name: 'Test Execution',
        hasSolution: vi.fn().mockReturnValue(true)
      ...
      expect(wrapper.find('[data-testid="input-output-data-table"]').exists()).toBe(true)
    })

    test('does not render InputOutputDataTable when execution has no solution', () => {
      const mockExecution = {
        id: 1,
        name: 'Test Execution',
      ...
      expect(wrapper.find('[data-testid="input-output-data-table"]').exists()).toBe(false)
    })

    test('does not render InputOutputDataTable when no execution is selected', () => {
      const { wrapper } = createWrapper()
  ...
  describe('Component Props', () => {
    test('passes correct props to MTitleView', () => {
      ...
      expect(titleView.props('title')).toBe('Output Data')
    ...
    test('passes selectedExecution and type to ExecutionInfoCard', () => {
      const mockExecution = {
        id: 1,
        name: 'Test Execution',
        hasSolution: vi.fn().mockReturnValue(true)
      ...
      expect(infoCard.props('type')).toBe('solution')
    })
    ...
    test('passes correct props to InputOutputDataTable when conditions are met', () => {
      const mockExecution = {
        id: 1,
        name: 'Test Execution',
        hasSolution: vi.fn().mockReturnValue(true)
      ...
      const { wrapper } = createWrapper(mockExecution)
      const dataTable = wrapper.findComponent({ name: 'InputOutputDataTable' })

      expect(dataTable.props('type')).toBe('solution')
  ...
  describe('Computed Properties', () => {
    test('title computed property returns correct value', () => {
      ...
      expect(wrapper.vm.title).toBe('Output Data')
    })
    ...
    test('hasSolution computed property calls selectedExecution.hasSolution', () => {
      const mockExecution = {
        id: 1,
        name: 'Test Execution',
        hasSolution: vi.fn().mockReturnValue(true)
      ...
      const result = wrapper.vm.hasSolution
      
      expect(mockExecution.hasSolution).toHaveBeenCalled()
      expect(result).toBe(true)
    })

    test('hasSolution computed property handles false return value', () => {
      const mockExecution = {
        id: 1,
        name: 'Test Execution',
        hasSolution: vi.fn().mockReturnValue(false)
  ...
  describe('Component Structure', () => {
    ...
    test('applies correct margin top class to InputOutputDataTable', () => {
      const mockExecution = {
        id: 1,
        name: 'Test Execution',
        hasSolution: vi.fn().mockReturnValue(true)
  ...
  describe('Edge Cases', () => {
    ...
    test('handles execution without hasSolution method gracefully', () => {
      const mockExecution = {
        id: 1,
        name: 'Test Execution'
        // No hasSolution method
      }
      
      // This should throw an error because the template tries to call hasSolution()
      expect(() => createWrapper(mockExecution)).toThrow()
    })
  })

  describe('Conditional Rendering Logic', () => {
    test('InputOutputDataTable visibility depends on execution and hasSolution', () => {
      const conditions = [
        { execution: null, visible: false },
        { execution: { id: 1, hasSolution: vi.fn().mockReturnValue(false) }, visible: false },
        { execution: { id: 1, hasSolution: vi.fn().mockReturnValue(true) }, visible: true }
      ]

      conditions.forEach(({ execution, visible }) => {
        const { wrapper } = createWrapper(execution)
        
        expect(wrapper.find('[data-testid="input-output-data-table"]').exists()).toBe(visible)
      })
    })
    ...
    test('ExecutionInfoMenu visibility depends on execution existence', () => {
      const conditions = [
        { execution: null, visible: false },
        { execution: { id: 1, name: 'Test', hasSolution: vi.fn().mockReturnValue(false) }, visible: true },
        { execution: { id: 1, hasSolution: vi.fn().mockReturnValue(true) }, visible: true }
      ]
  ...
  describe('Type-specific Features', () => {
    test('passes solution type to ExecutionInfoCard', () => {
      const mockExecution = {
        id: 1,
        name: 'Test Execution',
        hasSolution: vi.fn().mockReturnValue(true)
    ...
    test('passes solution type to InputOutputDataTable', () => {
      const mockExecution = {
        id: 1,
        name: 'Test Execution',
        hasSolution: vi.fn().mockReturnValue(true)
  ...
  describe('Methods Configuration', () => {
    test('has empty methods object', () => {
      ...
      // The component should have an empty methods object
