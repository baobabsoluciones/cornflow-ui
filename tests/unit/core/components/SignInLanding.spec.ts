...
describe('SignInLanding', () => {
  ...
  describe('Two-Factor Authentication Flow', () => {
    test('shows the MFA code step when the login requires a code', async () => {
      ...
      expect(vm.loginStep).toBe('mfa-code')
      expect(vm.totpCode).toBe('')
    })

    test('submits the TOTP code and finishes the login on success', async () => {
      const routerPushSpy = vi.spyOn(router, 'push')
      wrapper = createWrapper()
    ...
    test('shows an invalid code error when the TOTP code is rejected', async () => {
      wrapper = createWrapper()
    ...
    test('starts the enrollment with a QR code when MFA setup is required', async () => {
      mockAuthServices.cornflowAuthService.login.mockResolvedValueOnce({
        success: false,
        mfaSetupRequired: true,
      })
      mockAuthServices.cornflowAuthService.mfaSetup.mockResolvedValueOnce({
        secret: 'BASE32SECRET',
      ...
      expect(mockAuthServices.cornflowAuthService.mfaSetup).toHaveBeenCalled()
      expect(mockQRCodeToDataURL).toHaveBeenCalledWith(
        'otpauth://totp/app:user?secret=BASE32SECRET',
        expect.any(Object),
      )
      expect(vm.enrollSecret).toBe('BASE32SECRET')
      expect(vm.qrDataUrl).toBe('data:image/png;base64,mock-qr')
      expect(vm.loginStep).toBe('mfa-enroll')
    })

    test('shows an error when the MFA setup cannot be started', async () => {
      mockAuthServices.cornflowAuthService.login.mockResolvedValueOnce({
        success: false,
        mfaSetupRequired: true,
      })
      mockAuthServices.cornflowAuthService.mfaSetup.mockResolvedValueOnce(null)
      wrapper = createWrapper()
      ...
      expect(mockShowSnackbar).toHaveBeenCalledWith(
        'logIn.mfa_setup_error',
        'error',
      )
      expect(vm.loginStep).toBe('credentials')
    })

    test('verifyEnrollment shows the backup codes on success', async () => {
      mockAuthServices.cornflowAuthService.mfaVerify.mockResolvedValueOnce({
        backupCodes: ['aaaa-bbbb', 'cccc-dddd'],
      })
      wrapper = createWrapper()

      const vm = wrapper.vm as any
      vm.loginStep = 'mfa-enroll'
      vm.enrollCode = '123456'

      await vm.verifyEnrollment()

      expect(mockAuthServices.cornflowAuthService.mfaVerify).toHaveBeenCalledWith(
        '123456',
      )
      expect(vm.backupCodes).toEqual(['aaaa-bbbb', 'cccc-dddd'])
      expect(vm.loginStep).toBe('mfa-backup')
    })

    test('verifyEnrollment shows an error for an invalid code', async () => {
      mockAuthServices.cornflowAuthService.mfaVerify.mockResolvedValueOnce(null)
      wrapper = createWrapper()

      const vm = wrapper.vm as any
      vm.loginStep = 'mfa-enroll'
      vm.enrollCode = '000000'
    ...
    test('verifyEnrollment requires a code', async () => {
      wrapper = createWrapper()
      ...
      expect(
        mockAuthServices.cornflowAuthService.mfaVerify,
    ...
    test('finishLogin after the backup codes navigates home', async () => {
      const routerPushSpy = vi.spyOn(router, 'push')
      wrapper = createWrapper()

      const vm = wrapper.vm as any
      vm.loginStep = 'mfa-backup'
      vm.backupCodes = ['aaaa-bbbb']
    ...
    test('redirects to the user settings when a password change is required', async () => {
      const routerPushSpy = vi.spyOn(router, 'push')
      mockAuthServices.cornflowAuthService.login.mockResolvedValueOnce({
        success: true,
        changePassword: true,
    ...
    test('backToCredentials resets the MFA state', () => {
      wrapper = createWrapper()

      const vm = wrapper.vm as any
      vm.loginStep = 'mfa-enroll'
      vm.totpCode = '123456'
      vm.enrollCode = '654321'
      vm.enrollSecret = 'SECRET'
      vm.qrDataUrl = 'data:image/png;base64,mock-qr'
      vm.backupCodes = ['aaaa-bbbb']
      ...
      expect(vm.loginStep).toBe('credentials')
      expect(vm.totpCode).toBe('')
      expect(vm.enrollCode).toBe('')
      expect(vm.enrollSecret).toBe('')
  ...
  describe('Password Recovery Flow', () => {
    test('the forgot link switches to the forgot step and shows the email field', async () => {
      wrapper = createWrapper()

      const forgotLink = wrapper.find('[data-test="forgot-link"]')
      ...
      expect(wrapper.find('[data-test="forgot-email-input"]').exists()).toBe(
        true,
      )
    })

    test('submitForgot requires an email', async () => {
      wrapper = createWrapper()
      ...
      expect(
        mockAuthServices.cornflowAuthService.requestPasswordReset,
      ).not.toHaveBeenCalled()
      expect(mockShowSnackbar).toHaveBeenCalledWith(
        'This field is required',
        'error',
    ...
    test('submitForgot shows an error and stays on the forgot step when the request fails', async () => {
      ...
      expect(vm.loginStep).toBe('forgot')
      expect(vm.forgotEmail).toBe('user@example.com')
    })

    test('submitForgot shows the server error snackbar on exceptions', async () => {
      mockAuthServices.cornflowAuthService.requestPasswordReset.mockRejectedValueOnce(
    ...
    test('backToCredentials clears the forgot email', () => {
      wrapper = createWrapper()
