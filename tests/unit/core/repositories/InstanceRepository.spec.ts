...
describe('InstanceRepository', () => {
  ...
  describe('getInstance', () => {
    ...
    test('should get instance by ID successfully', async () => {
      ...
      expect(mockClient.get).toHaveBeenCalledWith(
        '/instance/test-instance-id/data/',
      )
    ...
    test('should throw error when API returns non-200 status', async () => {
      ...
      await expect(repository.getInstance('invalid-id')).rejects.toThrow(
    ...
    test('should handle API call failure', async () => {
      ...
      await expect(repository.getInstance('test-instance-id')).rejects.toThrow(
        'Network error',
      )
    })
  ...
  describe('launchInstanceDataChecks', () => {
    ...
    test('should throw error when API returns non-201 status', async () => {
      ...
      await expect(
        repository.launchInstanceDataChecks('invalid-id'),
    ...
    test('should throw error with unknown error message for non-201 status', async () => {
      ...
      await expect(
        repository.launchInstanceDataChecks('test-instance-id'),
      ).rejects.toThrow('Error launching instance data checks: Status 500')
    })
