...
test.describe('Input Data Page', () => {
  ...
  test('should show current plan FAB icon when the execution is the current plan', async ({ page }) => {
    await page.goto('/', { waitUntil: 'domcontentloaded' });
    const app = page.locator('.v-application');
    await app.first().waitFor({ state: 'visible', timeout: TIMEOUTS.FORM_LOAD });
    ...
    // Ensure there is a selected execution tab (the "Actual" execution should be loaded)
    const tablist = page.getByRole('tablist');
    await tablist.waitFor({ state: 'visible', timeout: TIMEOUTS.FORM_LOAD });
    ...
    // Navigate to the Input Data page
    const inputDataLink = page.locator('a[href*="input-data"]');
    await inputDataLink.first().waitFor({ state: 'visible', timeout: TIMEOUTS.FORM_LOAD });
    await inputDataLink.first().click();

    await expect(page).toHaveURL(/input-data/, { timeout: TIMEOUTS.NAVIGATION });

    // Wait for the Input Data page to load
    const sectionView = page.locator('.section-view');
    await sectionView.waitFor({ state: 'visible', timeout: TIMEOUTS.FORM_LOAD });

    // Check if the current plan FAB is visible
    const currentPlanFab = page.locator('.set-current-plan-fab');
    const fabVisible = await currentPlanFab.isVisible({ timeout: TIMEOUTS.FORM_LOAD }).catch(() => false);

    if (!fabVisible) {
      console.log('Current plan FAB is not visible - feature may not be available');
      return;
    }

    console.log('Current plan FAB is visible');

    // Check if the execution is the current plan (should have .is-current class)
    const fabButton = currentPlanFab.locator('.fab-button');
    await fabButton.waitFor({ state: 'visible', timeout: TIMEOUTS.FORM_LOAD });

    const isCurrentPlan = await fabButton.evaluate((el) => el.classList.contains('is-current'));
    
    if (isCurrentPlan) {
      console.log('Execution is the current plan - verifying icon');
      
      // Verify the star icon (mdi-star) is shown
      const starIcon = fabButton.locator('.mdi-star');
      await expect(starIcon).toBeVisible();
      console.log('Current plan icon (mdi-star) is visible');

      // Verify clicking does nothing (cursor is default)
      const cursorStyle = await fabButton.evaluate((el) => window.getComputedStyle(el).cursor);
      expect(cursorStyle).toBe('default');
      console.log('FAB button has default cursor (not clickable)');
    } else {
      console.log('Execution is NOT the current plan - this test is for current plan only');
      // Still verify the FAB is visible
      await expect(fabButton).toBeVisible();
    }

    console.log('Current plan FAB test completed successfully');
  });

  test('should allow setting an execution as current plan', async ({ page }) => {
    await page.goto('/', { waitUntil: 'domcontentloaded' });
    const app = page.locator('.v-application');
    await app.first().waitFor({ state: 'visible', timeout: TIMEOUTS.FORM_LOAD });

    const loadedExecution = await loadNonCurrentExecutionFromHistory(page);
    if (!loadedExecution) {
      console.log('Could not find a non-current plan execution to load');
      console.log('This test requires at least one execution that is NOT the current plan');
      return;
    }

    // Wait for the execution to be loaded (tab should appear)
    ...
    if (!fabVisible) {
      console.log('Current plan FAB is not visible - feature may not be available or execution state not valid');
      return;
    }

    // Verify the FAB button is visible
    const fabButton = currentPlanFab.locator('.fab-button');
    await fabButton.waitFor({ state: 'visible', timeout: TIMEOUTS.FORM_LOAD });

    // Check if this execution is already the current plan
    const isCurrentPlan = await fabButton.evaluate((el) => el.classList.contains('is-current'));

    if (isCurrentPlan) {
      console.log('Loaded execution is already the current plan - test scenario not applicable');
      return;
    }

    console.log('Execution is NOT the current plan - proceeding to set it as current');

    // Verify the star-plus-outline icon is shown
    const starPlusIcon = fabButton.locator('.mdi-star-plus-outline');
    await expect(starPlusIcon).toBeVisible();
    console.log('Star plus outline icon (mdi-star-plus-outline) is visible');

    // Click on the FAB to open the modal
    await fabButton.click();
    console.log('Clicked on FAB button');
    ...
    const modalTitle = modal.first().getByText(/Establecer como plan actual|Set as current plan/i);
    await expect(modalTitle).toBeVisible();
    console.log('Modal title verified');

    // Verify the confirm button is visible
    const confirmButton = modal.first().getByRole('button', { name: /Establecer como actual|Set as current|Confirm/i });
    ...
    // Verify the FAB now shows the current plan icon
    await fabButton.waitFor({ state: 'visible', timeout: TIMEOUTS.FORM_LOAD });
    
    // Wait for the state to update
    await page.waitForTimeout(1000);

    // Verify the icon changed to mdi-star
    const starIcon = fabButton.locator('.mdi-star');
    await expect(starIcon).toBeVisible({ timeout: TIMEOUTS.FORM_LOAD });
    console.log('Star icon (mdi-star) is now visible');

    // Verify the button now has the is-current class
    const isNowCurrentPlan = await fabButton.evaluate((el) => el.classList.contains('is-current'));
    expect(isNowCurrentPlan).toBe(true);
    console.log('FAB button now has is-current class');

    console.log('Successfully set execution as current plan');
  });
});
