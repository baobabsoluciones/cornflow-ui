...
test.describe('Version History Navigation', () => {
  ...
  test('should click set as current plan button star icon', async ({ page }) => {
    // Navigate to the version history page first
    const currentHash = getHashRoute(page);
    ...
    // ProjectExecutionsTable.vue: .set-latest-icon only when the row can be set as latest plan
    let firstRow: Locator | null = null;
    let setCurrentPlanButton: Locator | null = null;
    const rowCount = await tableRows.count();

    const rowWithSetLatest = tableRows
      .filter({ has: page.locator('.set-latest-icon') })
      .first();
    if (await rowWithSetLatest.isVisible({ timeout: 3000 }).catch(() => false)) {
      firstRow = rowWithSetLatest;
      setCurrentPlanButton = rowWithSetLatest.locator('.set-latest-icon').first();
    }

    // Fallback: scan rows for mdi-star-outline (not current plan)
    if (!firstRow) {
    for (let i = 0; i < rowCount; i++) {
      ...
      if (cellCount > 0) {
        ...
        // Check if this row has mdi-star-outline (not the current plan)
        // Look for mdi-star-outline icon in this row
        const classPatterns = [
          '[class*="mdi-star-outline"]',
          '.mdi-star-outline',
          'i.mdi-star-outline',
          '.v-icon.mdi-star-outline',
          '[class*="mdi"][class*="star-outline"]'
        ];
        
        let foundStarOutline = false;
        for (const pattern of classPatterns) {
          const starIcon = row.locator(pattern).first();
          const iconExists = await starIcon.count().catch(() => 0);
          if (iconExists > 0) {
            const isIconVisible = await starIcon.isVisible({ timeout: 500 }).catch(() => false);
            if (isIconVisible) {
              // Verify it's actually mdi-star-outline and not mdi-star (current plan)
              const iconClass = await starIcon.getAttribute('class').catch(() => '');
              if (iconClass) {
                // Make sure it has mdi-star-outline and doesn't have mdi-star without outline (current plan)
                const hasOutline = iconClass.includes('mdi-star-outline');
                const hasFilledStar = iconClass.includes('mdi-star') && !iconClass.includes('outline') && !iconClass.includes('plus');
                if (hasOutline && !hasFilledStar) {
                  foundStarOutline = true;
                  firstRow = row;
                  setCurrentPlanButton = starIcon;
                  break;
                }
              }
            }
        ...
        // Also check all icons in the row to be more thorough
        if (!foundStarOutline) {
          const allIconsInRow = row.locator('.v-icon, i[class*="mdi"], [class*="mdi"]');
          const iconCount = await allIconsInRow.count();
          
          for (let j = 0; j < iconCount; j++) {
            const icon = allIconsInRow.nth(j);
            const iconClass = await icon.getAttribute('class').catch(() => '');
            if (iconClass && iconClass.includes('mdi-star-outline') && !iconClass.includes('mdi-star-plus')) {
              // Verify it's not the filled star (current plan)
              const hasFilledStar = iconClass.includes('mdi-star') && !iconClass.includes('outline') && !iconClass.includes('plus');
              if (!hasFilledStar) {
                const isIconVisible = await icon.isVisible({ timeout: 500 }).catch(() => false);
                if (isIconVisible) {
                  foundStarOutline = true;
                  firstRow = row;
                  setCurrentPlanButton = icon;
                  break;
                }
              }
            }
    ...
    if (!firstRow || !setCurrentPlanButton) {
      // If no valid rows found, check for "no data" message
      const noDataMessage = page.getByText(/no hay datos|no data|aucune donnée|no se encontraron datos/i);
      const noDataExists = await noDataMessage.count().catch(() => 0);
      if (noDataExists > 0) {
        await expect(noDataMessage.first()).toBeVisible({ timeout: TIMEOUTS.FORM_LOAD });
      }
      console.log(
        'Skipping: no row can be set as current plan (all executions may already be the latest plan, state may not allow it, or the table has no data).'
      );
      return;
    }

    // Wait for the valid row to be fully rendered with all cells
    await firstRow.waitFor({ state: 'visible', timeout: TIMEOUTS.FORM_LOAD });
    
    // Wait for cells to be rendered in the row (excluding colspan cells)
    const rowCells = firstRow.locator('td:not([colspan]), td:not([colSpan])');
    await rowCells.first().waitFor({ state: 'visible', timeout: TIMEOUTS.FORM_LOAD });
    
    // Wait a bit more for all cells and icons to be fully rendered
    await page.waitForTimeout(1000);
    
    // Strategy 1: Find by looking for the Actions column header first, then find the corresponding cell
    const actionsHeader = page.locator('th, .v-data-table-header th, thead th').filter({
      hasText: /Acciones|Actions|Actions/i
    }).first();
    ...
    if (actionsHeaderExists > 0) {
      ...
      if (actionsColumnIndex >= 0) {
        ...
        if (cellExists > 0) {
          ...
          // Look for the icon with mdi-star-outline class in this cell
          ...
          const classPatterns = [
            ...
            '.v-icon.mdi-star-outline',
            '[class*="mdi"][class*="star-outline"]',
            '.v-icon:has([class*="mdi-star-outline"])'
          ];
          
          for (const pattern of classPatterns) {
            const starIcon = actionsCell.locator(pattern).first();
            const iconExists = await starIcon.count().catch(() => 0);
            if (iconExists > 0) {
              const isVisible = await starIcon.isVisible({ timeout: 500 }).catch(() => false);
              if (isVisible) {
                setCurrentPlanButton = starIcon;
                break;
              }
          ...
          // Also try to find in the actions-container
          if (!setCurrentPlanButton) {
            const actionsContainer = actionsCell.locator('.actions-container').first();
            const containerExists = await actionsContainer.count().catch(() => 0);
            if (containerExists > 0) {
              for (const pattern of classPatterns) {
                const starIcon = actionsContainer.locator(pattern).first();
                const iconExists = await starIcon.count().catch(() => 0);
                if (iconExists > 0) {
                  const isVisible = await starIcon.isVisible({ timeout: 500 }).catch(() => false);
                  if (isVisible) {
    ...
    // Strategy 2: Direct search for mdi-star-outline in the row with multiple class patterns
    if (!setCurrentPlanButton) {
      const classPatterns = [
        '[class*="mdi-star-outline"]',
        '.mdi-star-outline',
      ...
      for (const pattern of classPatterns) {
        ...
        if (directSearchCount > 0) {
          const isVisible = await directSearch.isVisible({ timeout: 500 }).catch(() => false);
          if (isVisible) {
            setCurrentPlanButton = directSearch;
            break;
          }
        }
      }
    ...
    if (!setCurrentPlanButton) {
      ...
      for (let i = 0; i < iconCount; i++) {
        const icon = allIconsInRow.nth(i);
        const iconClass = await icon.getAttribute('class').catch(() => '');
        if (iconClass && iconClass.includes('mdi-star-outline')) {
          const isVisible = await icon.isVisible({ timeout: 500 }).catch(() => false);
          if (isVisible) {
            setCurrentPlanButton = icon;
            break;
    ...
    // Strategy 4: Search in the entire table for the icon
    if (!setCurrentPlanButton) {
      const tableIcons = tableContainer.locator('[class*="mdi-star-outline"]').first();
      const tableIconCount = await tableIcons.count().catch(() => 0);
      if (tableIconCount > 0) {
        const isVisible = await tableIcons.isVisible({ timeout: 500 }).catch(() => false);
        if (isVisible) {
          setCurrentPlanButton = tableIcons;
        }
      }
    }
    
    // Strategy 5: Try hovering over the row first, then search (some icons might need hover to be visible)
    if (!setCurrentPlanButton) {
      await firstRow.hover().catch(() => {});
      await page.waitForTimeout(300);
      
      // Try the same searches again after hover
      const classPatterns = [
        '[class*="mdi-star-outline"]',
        '.mdi-star-outline',
        'i.mdi-star-outline',
    ...
    // Strategy 6: Search by tooltip text (Establecer como plan actual, Set as current plan, etc.)
    if (!setCurrentPlanButton) {
      // Look for elements with tooltip containing "Establecer como plan actual" or "Set as current plan"
      const tooltipElements = page.locator('[title*="Establecer como plan actual"], [title*="Set as current plan"], [aria-label*="Establecer como plan actual"], [aria-label*="Set as current plan"]').first();
      const tooltipExists = await tooltipElements.count().catch(() => 0);
      if (tooltipExists > 0) {
        // Get the parent that contains the icon
        const parentWithIcon = tooltipElements.locator('..').locator('[class*="mdi-star-outline"]').first();
        const parentIconExists = await parentWithIcon.count().catch(() => 0);
        if (parentIconExists > 0) {
          setCurrentPlanButton = parentWithIcon;
        } else {
          // Maybe the tooltip element itself is the icon container
          const iconInTooltip = tooltipElements.locator('[class*="mdi-star-outline"]').first();
          const iconInTooltipExists = await iconInTooltip.count().catch(() => 0);
          if (iconInTooltipExists > 0) {
            setCurrentPlanButton = iconInTooltip;
          }
        }
      }
    }

    // Verify the set current plan button was found
    if (!setCurrentPlanButton) {
      // Debug: Log the row HTML to understand what's available
      const rowHTML = await firstRow.innerHTML().catch(() => '');
      console.log('Row HTML (first 1000 chars):', rowHTML.substring(0, 1000));
      
      // Also check what icons are in the row
      const allIcons = firstRow.locator('.v-icon, i, [class*="mdi"], [class*="icon"]');
      const iconCount = await allIcons.count();
      ...
      for (let i = 0; i < Math.min(iconCount, 15); i++) {
        ...
        const iconTag = await icon.evaluate(el => el.tagName).catch(() => '');
        const isVisible = await icon.isVisible().catch(() => false);
        console.log(`Icon ${i}: tag="${iconTag}", class="${iconClass}", text="${iconText}", visible=${isVisible}`);
      }
      
      // Also check the actions cell specifically
      const actionsHeader = page.locator('th, .v-data-table-header th, thead th').filter({
        hasText: /Acciones|Actions/i
      }).first();
      const actionsHeaderExists = await actionsHeader.count().catch(() => 0);
      if (actionsHeaderExists > 0) {
        const allHeaders = page.locator('th, .v-data-table-header th, thead th');
        ...
        for (let i = 0; i < headerCount; i++) {
          const header = allHeaders.nth(i);
          const headerText = await header.textContent().catch(() => '');
          if (headerText && /Acciones|Actions/i.test(headerText.trim())) {
            actionsColumnIndex = i;
            break;
          }
        }
        ...
        if (actionsColumnIndex >= 0) {
          const actionsCell = rowCells.nth(actionsColumnIndex);
          const actionsCellHTML = await actionsCell.innerHTML().catch(() => '');
          console.log('Actions cell HTML:', actionsCellHTML.substring(0, 500));
        }
      }
      
      console.log(
        'Skipping: could not resolve "set as current plan" control after finding a candidate row (Actions column / icon layout may differ).'
      );
      return;
    }

    // Wait for the button to be visible and clickable
    await setCurrentPlanButton.waitFor({ state: 'visible', timeout: TIMEOUTS.FORM_LOAD });

    // Verify the button is clickable
    await expect(setCurrentPlanButton).toBeVisible();
    
    // Click on the set current plan button
    await setCurrentPlanButton.click();

    // Wait for the modal to appear
    await page.waitForTimeout(1000);

    // Verify that the modal appears after clicking
    // The modal for setting current plan should appear
    ...
    // Verify the modal title "Establecer como plan actual" / "Set as current plan"
    const modalTitle = modal.locator('h2, .v-card-title, [class*="title"], .modal-title').filter({
      hasText: /Establecer como plan actual|Set as current plan|Définir comme plan actuel/i
    }).first();
    await expect(modalTitle).toBeVisible({ timeout: TIMEOUTS.FORM_LOAD });
    
    // Verify the star icon is present in the modal
    const starIcon = modal.locator('[class*="mdi-star"]:not([class*="outline"])').first();
    await expect(starIcon).toBeVisible({ timeout: TIMEOUTS.FORM_LOAD });
    
    // Verify the confirmation message is present
    const confirmationMessage = modal.locator('.modal-message, p').filter({
      hasText: /¿Estás seguro|Are you sure|Êtes-vous sûr|Se cargará automáticamente|will be loaded automatically|sera chargé automatiquement/i
    }).first();
    await expect(confirmationMessage).toBeVisible({ timeout: TIMEOUTS.FORM_LOAD });
    
    // Verify the execution info section is present (EJECUCIÓN label and execution name)
    const executionInfo = modal.locator('.execution-info').first();
    await expect(executionInfo).toBeVisible({ timeout: TIMEOUTS.FORM_LOAD });
    
    // Verify the execution label "EJECUCIÓN" / "EXECUTION"
    const executionLabel = executionInfo.locator('.execution-info-label, [class*="label"]').filter({
      hasText: /EJECUCIÓN|EXECUTION|EXÉCUTION/i
    }).first();
    await expect(executionLabel).toBeVisible({ timeout: TIMEOUTS.FORM_LOAD });
    
    // Verify the execution name is displayed
    const executionNameValue = executionInfo.locator('.execution-info-value, [class*="value"]').first();
    await expect(executionNameValue).toBeVisible({ timeout: TIMEOUTS.FORM_LOAD });
    const executionNameText = await executionNameValue.textContent();
    expect(executionNameText).toBeTruthy();
    expect(executionNameText?.trim().length).toBeGreaterThan(0);
    
    // Verify the warning message (if there's a current plan to replace)
    // This might not always be present, so we check if it exists
    const warningMessage = modal.locator('.current-plan-warning, [class*="warning"]').first();
    const warningExists = await warningMessage.count().catch(() => 0);
    if (warningExists > 0) {
      const isWarningVisible = await warningMessage.isVisible({ timeout: 1000 }).catch(() => false);
      if (isWarningVisible) {
        // Verify the warning icon
        const warningIcon = warningMessage.locator('[class*="mdi-information"]').first();
        await expect(warningIcon).toBeVisible({ timeout: 1000 });
        
        // Verify the warning text mentions replacing a plan
        const warningText = await warningMessage.textContent();
        expect(warningText).toMatch(/reemplazará|replace|remplacer/i);
      }
    }
    
    // Verify the action buttons are present
    // Button "Establecer como actual" / "Set as current"
    const confirmButton = modal.locator('button, .v-btn').filter({
      hasText: /Establecer como actual|Set as current|Définir comme actuel/i
    }).first();
    await expect(confirmButton).toBeVisible({ timeout: TIMEOUTS.FORM_LOAD });
    
    // Button "Cancelar" / "Cancel"
    const cancelButton = modal.locator('button, .v-btn').filter({
      hasText: /Cancelar|Cancel|Annuler/i
    }).first();
    await expect(cancelButton).toBeVisible({ timeout: TIMEOUTS.FORM_LOAD });
    
    // Verify the close button (X) is present
    const closeButton = modal.locator('button:has([class*="mdi-close"]), [aria-label*="close"], [aria-label*="cerrar"]').first();
    ...
    // Store the execution name before clicking confirm (to verify it later)
    const executionNameBefore = await executionNameValue.textContent();

    // Click on the "Establecer como actual" button
    await confirmButton.click();

    // Immediately wait for the success snackbar to appear (it may disappear quickly)
    // Use a more aggressive wait strategy to catch it before it disappears
    const successSnackbar = page.locator('.v-snackbar, [class*="snackbar"], [role="alert"]').filter({ 
      hasText: /Plan actual establecido con éxito|Current plan set successfully|Plan actuel défini avec succès/i
    }).first();
    
    // Wait for the snackbar to appear with a shorter timeout but more frequent checks
    try {
      await successSnackbar.waitFor({ state: 'visible', timeout: 5000 });
    } catch (e) {
      // If it doesn't appear immediately, try waiting a bit and checking again
      await page.waitForTimeout(100);
      const snackbarExists = await successSnackbar.count().catch(() => 0);
      if (snackbarExists === 0) {
        // Try to find any snackbar and check its text
        const anySnackbar = page.locator('.v-snackbar, [class*="snackbar"], [role="alert"]').first();
        const anySnackbarExists = await anySnackbar.count().catch(() => 0);
        if (anySnackbarExists > 0) {
          const anySnackbarText = await anySnackbar.textContent().catch(() => '');
          console.log('Found snackbar with text:', anySnackbarText);
        }
        throw new Error('Success snackbar did not appear after clicking "Establecer como actual" button.');
      }
    }
    
    // Verify the snackbar is visible
    await expect(successSnackbar).toBeVisible({ timeout: 1000 });
    
    // Verify the snackbar text content immediately (before it might disappear)
    const snackbarText = await successSnackbar.textContent();
    expect(snackbarText).toMatch(/Plan actual establecido con éxito|Current plan set successfully|Plan actuel défini avec succès/i);

    // Wait for the API call to complete and modal to close
    await page.waitForTimeout(1000);

    // Verify the modal is closed (should not be visible)
    ...
    // Wait a bit for the UI to update (icon change)
    await page.waitForTimeout(1000);

    // Verify the icon in the table row has changed from outline to filled (mdi-star instead of mdi-star-outline)
    // The icon should now be mdi-star (filled) instead of mdi-star-outline
    let filledStarIcon: Locator | null = null;
    
    // Strategy 1: Find the filled star icon in the same row
    const filledStarPatterns = [
      '[class*="mdi-star"]:not([class*="outline"]):not([class*="plus"])',
      '.mdi-star:not(.mdi-star-outline):not(.mdi-star-plus)',
      'i.mdi-star:not([class*="outline"]):not([class*="plus"])',
      '.v-icon.mdi-star:not([class*="outline"])'
    ];
    
    for (const pattern of filledStarPatterns) {
      const filledStar = firstRow.locator(pattern).first();
      const iconExists = await filledStar.count().catch(() => 0);
      if (iconExists > 0) {
        const isVisible = await filledStar.isVisible({ timeout: 500 }).catch(() => false);
        if (isVisible) {
          const iconClass = await filledStar.getAttribute('class').catch(() => '');
          // Make sure it's the filled star, not outline
          if (iconClass && iconClass.includes('mdi-star') && !iconClass.includes('outline') && !iconClass.includes('plus')) {
            filledStarIcon = filledStar;
            break;
          }
        }
      }
    ...
    // Strategy 2: Find all icons in the row and check for filled star
    if (!filledStarIcon) {
      const allIconsInRow = firstRow.locator('.v-icon, i[class*="mdi"], [class*="mdi"]');
      const iconCount = await allIconsInRow.count();
    ...
    // Strategy 3: Check in the actions cell specifically
    if (!filledStarIcon) {
      const actionsHeader = page.locator('th, .v-data-table-header th, thead th').filter({
        hasText: /Acciones|Actions/i
      }).first();
      ...
      if (actionsHeaderExists > 0) {
        ...
        if (actionsColumnIndex >= 0) {
          ...
          if (cellExists > 0) {
            for (const pattern of filledStarPatterns) {
              const filledStar = actionsCell.locator(pattern).first();
              const iconExists = await filledStar.count().catch(() => 0);
              if (iconExists > 0) {
                const isVisible = await filledStar.isVisible({ timeout: 500 }).catch(() => false);
                if (isVisible) {
    ...
    // Strategy 4: Search by tooltip text (should now say "Este es el Plan Actual" / "This is the current plan")
    if (!filledStarIcon) {
      const tooltipElements = page.locator('[title*="Este es el Plan Actual"], [title*="This is the current plan"], [title*="Este es el plan actual"], [aria-label*="Este es el Plan Actual"], [aria-label*="This is the current plan"]').first();
      const tooltipExists = await tooltipElements.count().catch(() => 0);
      if (tooltipExists > 0) {
        // Get the parent that contains the icon
        const parentWithIcon = tooltipElements.locator('..').locator('[class*="mdi-star"]:not([class*="outline"]):not([class*="plus"])').first();
        const parentIconExists = await parentWithIcon.count().catch(() => 0);
        if (parentIconExists > 0) {
          filledStarIcon = parentWithIcon;
        } else {
          // Maybe the tooltip element itself is the icon container
          const iconInTooltip = tooltipElements.locator('[class*="mdi-star"]:not([class*="outline"]):not([class*="plus"])').first();
          const iconInTooltipExists = await iconInTooltip.count().catch(() => 0);
          if (iconInTooltipExists > 0) {
            filledStarIcon = iconInTooltip;
          }
        }
      }
    }

    // Verify the filled star icon was found
    if (!filledStarIcon) {
      // Debug: Log the row HTML to understand what's available
      const rowHTML = await firstRow.innerHTML().catch(() => '');
      console.log('Row HTML after setting plan (first 1000 chars):', rowHTML.substring(0, 1000));
      
      // Also check what icons are in the row
      const allIcons = firstRow.locator('.v-icon, i, [class*="mdi"], [class*="icon"]');
      const iconCount = await allIcons.count();
      console.log(`Found ${iconCount} icons in the row after setting plan`);
      ...
      throw new Error('Could not find filled star icon (mdi-star) in table row after setting as current plan. The icon should have changed from mdi-star-outline to mdi-star.');
    }

    // Verify the filled star icon is visible
    await expect(filledStarIcon).toBeVisible({ timeout: TIMEOUTS.FORM_LOAD });
    
    // Verify it's the filled star (not outline)
    const iconClass = await filledStarIcon.getAttribute('class');
    expect(iconClass).toContain('mdi-star');
    expect(iconClass).not.toContain('mdi-star-outline');
    expect(iconClass).not.toContain('mdi-star-plus');
  });
  ...
  test('should close modal when clicking cancel button', async ({ page }) => {
    ...
    // Find and click the "Cancelar" / "Cancel" button
    const cancelButton = modal.locator('button, .v-btn').filter({
      hasText: /Cancelar|Cancel|Annuler/i
    }).first();
    await expect(cancelButton).toBeVisible({ timeout: TIMEOUTS.FORM_LOAD });
