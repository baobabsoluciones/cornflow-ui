...
import {
  ...
  skipSessionAuthInjectOnNextNavigation,
} from '../authInjectSkip';
...
export async function cornflowAuth(
  ...
  // Avoid fixtures.ts re-injecting setup sessionStorage on load (would bypass real login).
  await skipSessionAuthInjectOnNextNavigation(page);

  // Navigate to sign-in page and wait for form
  await page.goto('/sign-in', { waitUntil: 'domcontentloaded' });
  // Wait for the form to be visible instead of networkidle (more reliable)
  await page.waitForSelector(SELECTORS.LOGIN_FORM, { timeout: TIMEOUTS.FORM_LOAD, state: 'visible' });
  ...
  // The setup project (auth.setup) does not use fixtures.ts addInitScript, so the skip flag
  // is never consumed on /sign-in. If it stays in localStorage it gets saved in storageState
  // and the next test's first navigation skips sessionStorage injection → no user in UI.
...
export async function logout(
  ...
  if (navigateToSignIn) {
    await skipSessionAuthInjectOnNextNavigation(page);
    await page.goto('/sign-in', { waitUntil: 'domcontentloaded' });
    // Wait for sign-in form to be ready instead of networkidle
    await page.waitForSelector(SELECTORS.LOGIN_FORM, { timeout: TIMEOUTS.FORM_LOAD, state: 'visible' }).catch(() => {
      // Form might not be visible if already logged out, just wait for page load
    });
  }
}
