    ...
    <div class="sign-in">
        <div class="modal">
            ...
            <div class="form-content">
                ...
                <v-text-field variant="outlined" density="compact" class="input" v-model="username"
                    style="margin-bottom: -8px;" />

                <label class="label">Contraseña</label>
                <v-text-field variant="outlined" density="compact" class="input" v-model="password"
                    :type="show ? 'text' : 'password'" :append-inner-icon="!show ? 'mdi-eye' : 'mdi-eye-off'"
                    @click:append-inner="show = !show" @keydown.enter.prevent="submitLogIn()" />
...
export default {
    ...
    methods: {
        async submitLogIn() {
            const isAuthenticated = await AuthService.login(this.username, this.password);
            if (isAuthenticated) {
                this.$router.push('/');
...
.logo {
    background-image: url('@/assets/logo_escudo_simple.png');
    background-repeat: no-repeat;
    background-size: 100% 100%;
    height: 40px;
    width: 41px;
