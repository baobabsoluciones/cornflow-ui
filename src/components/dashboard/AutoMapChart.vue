...
const frame = ref<InstanceType<typeof MapChartFrame> | null>(null)
const mapContainer = computed<HTMLElement | null>(
  () => frame.value?.mapContainer ?? null,
)
...
<style scoped>
/*
 * The chart-card / map-container / overlay markup and its styles now live in
 * the shared MapChartFrame component. Only the Leaflet marker reset (which
 * targets a chart-specific class injected at runtime) stays here.
 */
:deep(.custom-marker) {
